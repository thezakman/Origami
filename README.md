...
- **Secrets, not just files.** Finding `/.env`, a config, a backup or a JS bundle is half the job — Origami reads the high-value ones and flags the **credentials inside**: AWS/Google/GitHub/GitLab/Slack/Stripe keys, **modern provider tokens** (OpenAI, Anthropic, DigitalOcean, Shopify, Square, Telegram, Azure Storage), private keys, JWTs, DB connection strings and guarded `api_key=…` assignments (doc placeholders rejected). Hits are tagged `secret` (loud), redacted in the report, and lead the end-of-scan triage line — so a bypassed 403 or a found dotfile immediately tells you *what leaked*.
...
- **Parameter discovery** (`--params`). Origami learns parameter *names* from JS and GraphQL — this fires them (plus a common-param list) at dynamic endpoints with unique canaries and flags the ones that **reflect**: a real input the app reads, and a lead for XSS / SSTI / open-redirect. Reflection is batched (many params per request, each canary maps back to its param) and a per-batch control param suppresses the "echoes any query" false positive. Crucially it **grades the reflection by injection context** — where the canary landed: HTML text, a tag attribute, inside `<script>` (JS), or a JSON echo — so a reflection into an HTML/JS sink is flagged `xss-lead` (not just `param`), turning a generic "reflects" into an actionable, prioritized finding.
...
- **403/401 bypass** (`--bypass-403`). A denial is evidence, not a dead end: each blocked resource gets a curated bypass battery — path tricks (dot/slash games, `..;/` matrix params, `%2e`/`%252f` encodings, the IIS backslash, extension spoofs), trust headers (**Cloudflare/edge `CF-Connecting-IP`, `Cluster-Client-IP`, `True-Client-IP`**, the `X-Forwarded-*` family, and the IIS/proxy URL-rewrite set `X-Original-URL`/`X-Rewrite-URL`), content-returning method swaps, **hop-by-hop bypass** (spoof a trusted value *and* name the header in `Connection:` so the edge proxy allows on it then strips it — a reverse-proxy-chain desync; plus pure-strip and a batch strip), **encoded-separator** path tricks (overlong-UTF-8 `%c0%af`, fullwidth `%ef%bc%8f`, IIS `%u` slashes a downstream normalizer decodes after the ACL check), and **API version-prefix** add/strip (`/admin` ↔ `/v1/admin`). Case tricks are skipped on case-insensitive hosts. A surviving 2xx **with content** that isn't the homepage **or the 403 page itself** (verified by simhash + soft-404 sibling) is reported as a real bypass — so you see which 403s actually hide something. The battery is **fingerprint-gated by default** (`auto`): the stack-specific families (encoded-separator, API-prefix) fire only where they fit, so a throttled scan doesn't waste budget — `--bypass-403 light` trims to the core, `--bypass-403 full` runs everything. Need more reach? `--bypass-headers` swaps the built-in header axis for a wordlist — the bundled `403-headers.txt` (250+ trust/override headers × loopback/internal spellings) by default, or `--bypass-headers your-list.txt` for your own `Header: value` file.
...
- **Cross-target memory** — SQLite corpus primes new scans from past ones; the n-gram completer improves as it grows. Hosts are keyed by registrable identity with `www`/apex collapsed to one entry (so `www.x.com` and `x.com` share what they learn); `--forget HOST|all` clears it.
