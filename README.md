...
- **Folds in the whole API surface.** Probes the common OpenAPI/Swagger spec locations (`/swagger.json`, `/openapi.json`, `/v3/api-docs`…) **and the JSON:API index** (`/jsonapi`, Drupal-style); when one parses, every declared path / resource link becomes a seed (with the `basePath`/`servers` prefix applied), surfacing endpoints no wordlist would guess.
