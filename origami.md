...
- async engine + backoff; soft-404 classifier with **`-mc/-fc/-ms/-fs`** status/size filters plus **body filters** (`--filter-word-count`/`--filter-line-count`/`--filter-regex`/`--filter-similar-to`, feroxbuster-style; 404/400 dropped by default) and random-sibling verification of surprising hits;
