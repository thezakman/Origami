...
- **403/401 bypass** (`--bypass-403`). A denial is evidence, not a dead end: each blocked resource gets curated path/header/method bypass tricks (nomore403-style); a surviving 2xx **with content** (verified against the homepage and soft-404 walls) is reported as a real bypass — so you see which 403s actually hide something.
