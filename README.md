...
- **Discovery that compounds (deep harvest).** Recon doesn't stop at the homepage: every JS/JSON/spec/HTML file the scan *itself* turns up is re-read for endpoints — a wordlist-found `/app/bundle.js` reveals `/app/api/v2/users` no wordlist would guess — and the new in-scope paths are calibrated and probed. The more it finds, the more it reads, the more it finds.
