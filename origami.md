...
- **js_parser** (JS→JS chunks/sourcemaps, skips vendor, `data-main`) + **parameter** intel; **backups/VCS**; **robots/sitemap**; **OpenAPI/Swagger + JSON:API** spec discovery; **`.well-known/`** (OIDC/OAuth auth-endpoint folding + security.txt);
