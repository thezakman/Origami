...
- **Directory-listing aware.** A 2xx directory whose body is a server autoindex (Apache/nginx/IIS/Tomcat) is tagged `listing` (loud in triage) — and instead of blindly brute-forcing it, Origami **parses the listing** and folds the directory's TRUE contents (files + subdirs, resolved) into the scan. Index-hidden files (dotfiles/backups/VCS) are still probed by the backup fold, since that's exactly what autoindex hides.
