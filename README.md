...
The final report groups findings by confidence, tagged by kind (`secret`, `leak`, `xss-lead`, `ssti-lead`, `redirect-lead`, `auth-bypass`, `odata-agg`, `disclosure`, `config`, `api`, `admin`, `auth`, `source`, `param`, `listing`, `vhost`, `bypass`…) and coloured by where each came from (`js`, `robots`, `backup`, `wordlist`, `memory`, `shortscan`, `wayback`, `apidocs`, `wellknown`, `graphql`, `odata`, `overlay`, `methods`, `origin`, `bypass403`…):
