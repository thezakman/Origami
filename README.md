...
3. **Discovery compounds.** It reads the target's own code — JS, API specs, robots, headers, archives — then re-reads every file *it* uncovers and recurses the directories they live in. Give it a deep URL (`…/reports/2024/q3-summary.pdf`) and it **climbs the path** — scans the file's directory (not the file-as-folder), fetches the file, walks every ancestor dir up to root, and folds each segment into the dynamic wordlist. The more it finds, the more it finds.
...
| `--deep` | aggressive-discovery preset: `--bypass-403 --cache-poison --probe-405 --buckets --params --wayback --origin` at once, plus shortscan in `--auto` (full 8.3 long-name recovery) — state-changing/off-host probes included. Just `origami --deep -u <url>` |
...
| `--curl` | after the scan, print a copy-paste **curl** to reproduce each finding — the exact `-H`/`-X` for a header/method bypass, the payload URL for an OData `?$top=1`, etc. |
...
| `--as 'label: Header: value'` | add a request **identity** for the authorization differential (BOLA/BFLA/broken-auth): the discovered surface is replayed under each identity and Origami flags where a lesser identity reaches what only the privileged session should. Repeatable (reuse a label for several headers). Auto-runs on any authenticated scan; `--no-anon` drops the implicit anon identity |
...
| `--shortscan` / `--no-shortscan` | force / disable the IIS 8.3 fold (auto when IIS detected). Under `--deep` shortscan runs in **`--auto`** — full long-name recovery → tier-0 seeds — rate-mirrored, `--maxtime`-bounded, throttle-gated |
...
### Access-control differential (`--as`) — from discovery to BOLA/BFLA

Origami maps the endpoint surface better than a blind fuzzer — and then turns that surface into **access-control findings**. Give it more than one identity and it **replays every discovered endpoint under each**, diffing the responses to flag where identities *converge* but should *diverge* (OWASP API #1/#3/#5):

```bash
origami https://app.example.com \
  -H 'Cookie: session=<admin>' \
  --as 'low:  Cookie: session=<low-priv-user>' \
  --as 'other: Cookie: session=<second-user>'
```

- **`broken-auth`** — the implicit **anonymous** identity reaches the *same content* as the authenticated session (missing authentication/authorization).
- **`bola-lead`** — a *distinct authenticated* identity sees the same object/content as the primary session (broken object/function-level authz — verify ownership).
- **`authz-diff`** — the privileged session is **denied** where a lesser identity is **served** (an access-control inconsistency).

It runs **automatically on any authenticated scan** (a free anon-vs-authed diff — `--no-anon` opts out) and whenever `--as` is given. Reuse a label to give one identity several headers (`--as 'low: Cookie: …' --as 'low: X-Api-Key: …'`). Convergence is gated to non-public resources so shared public pages aren't flagged, and *per-user divergent* bodies are treated as correct scoping — not a bug. **Read-only GETs**: nothing is forged or replayed onward. It's the Autorize/Auth-Analyzer idea, native to the discovery engine and anchored on the surface it just found.
...
The final report groups findings by confidence, tagged by kind (`secret`, `leak`, `xss-lead`, `ssti-lead`, `redirect-lead`, `auth-bypass`, `odata-agg`, `jwt`, `oauth`, `broken-auth`, `bola-lead`, `authz-diff`, `negotiation`, `disclosure`, `config`, `api`, `admin`, `auth`, `source`, `param`, `listing`, `vhost`, `bypass`…) and coloured by where each came from (`js`, `robots`, `backup`, `wordlist`, `memory`, `shortscan`, `wayback`, `apidocs`, `wellknown`, `graphql`, `odata`, `authz`, `overlay`, `methods`, `origin`, `bypass403`…):

```
Findings (10)  ·  247 requests
┏━━━━━━┳━━━━━━┳━━━━━━━━━━━┳━━━━━━┳━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃ code ┃ size ┃ src       ┃ conf ┃ tags            ┃ path                               ┃
┡━━━━━━╇━━━━━━╇━━━━━━━━━━━╇━━━━━━╇━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ 200  │ 108B │ methods   │ 0.70 │ config          │ / (dangerous methods: DELETE, PUT) │
│ 200  │   0B │ graphql   │ 0.90 │ api auth-bypass │ /graphql (introspection enabled)   │
│ 200  │  15B │ js        │ 0.95 │ api admin       │ /api/v2/admin/secret               │
│ 200  │  52B │ robots    │ 0.95 │                 │ /private/dashboard.aspx            │
│ 200  │ 148B │ apidocs   │ 0.95 │ api             │ /swagger.json                      │
│ 200  │  15B │ apidocs   │ 0.95 │ api debug       │ /api/v3/internal/metrics           │
│ 200  │  41B │ wellknown │ 0.95 │ auth            │ /oauth2/authorize                  │
│ 200  │  21B │ backup    │ 0.95 │ disclosure      │ /.git/HEAD                         │
│ 200  │ 167B │ backup    │ 0.95 │ disclosure      │ /.env                              │
│ 403  │  48B │ overlay   │ 0.85 │                 │ /web.config                        │
└──────┴──────┴───────────┴──────┴─────────────────┴────────────────────────────────────┘
```
...
- **API surface** — OpenAPI/Swagger + JSON:API + `.well-known/` (OIDC/OAuth), **multi-document aware** (parses the Swagger-UI `urls:[…]` config and folds *every* declared spec — a .NET app serving `internal`/`siscomexEvents`/`codebaEvents` under one UI gets all three, incl. custom per-area paths a brute list misses); or hand it a spec with `--openapi URL|FILE` (`--swagger`/`--spec`).
...
- **JWT & OAuth weakness detection** (passive) — when a response hands over a **JWT** (a `{"token":…}` body field, a `Set-Cookie` session, a `WWW-Authenticate` header) Origami decodes it and flags `alg:none`, `kid` path-traversal / URL-injection, `jku`/`x5u` remote-key URLs (SSRF / key injection), `x5c` key-confusion, missing `exp`, and privilege claims (`role`/`admin`/`scope`) — tagged `jwt`. When it sees an **OAuth authorize URL** it flags missing `state` (CSRF), a PKCE `plain` downgrade, or no PKCE at all — tagged `oauth`. Read-only: nothing is forged or replayed; it's the recon lead a token attack starts from.
- **IIS 8.3 shortscan** — drives [`shortscan`](https://github.com/thezakman/shortscan), expands leaks in confidence tiers, and completes truncated names with an n-gram model (`APIINT~1` → `apiintegracao`). Under `--deep` it runs shortscan in **`--auto`** — the full long-name recovery cascade (siblings, references, tilde-continuation, collision/checksum, brute stem-completion) — so the tool reconstructs the whole names itself and they fold in as tier-0 seeds; rate-mirrored, `--maxtime`-bounded and skipped while the target is throttling, with any source-disclosure loot kept out of the CWD.
...
On top of that core: **content intelligence** (secrets — incl. modern provider tokens — plus stack-trace/debug-page/internal-infra disclosure), **parameter discovery with verified injection leads** (`--params` — reflection graded and *proven* with a breakout probe: unescaped `xss-lead`, `{{7*7}}→49` `ssti-lead`, `Location`/header `redirect-lead`), **GraphQL schema-mining** (introspection → args + sensitive-op flagging + a queries-only unauth probe → `auth-bypass`/BOLA leads), **OData schema-mining + query-option exposure** (`$metadata`/EDMX → entity sets folded as seeds + property/param harvest + sensitive-surface flagging; and on *any* discovered API collection — no `$metadata` needed — a **read-only** `$apply=aggregate($count)` + `$top=1` probe that flags authorization-bypass data exposure: a count or a row returned without auth, *especially where the plain listing is blocked* (413/403) but OData paging/aggregation walk around it → `odata-agg`/`auth-bypass`), **web cache poisoning** (`--cache-poison` — passive cache-layer fingerprint + safe unkeyed-input probing with throwaway cache-busters), **historical-URL sourcing** (`--wayback`/`--gau`), **virtual-host discovery** (`--vhost`), **origin-IP discovery + IP-based WAF bypass** (`--origin` — crt.sh/Shodan/SecurityTrails/Censys → direct-IP probing behind the CDN), **403/401 bypass** (`--bypass-403` with fingerprint-gated `light|auto|full` intensity — path/case tricks, character percent-encoding, normalization-diff + traversal-resolve, hop-by-hop, encoded-separator, API-prefix, matrix-management; **learns the WAF's weakness** and fires the winning trick first; `--bypass-headers`/`--bypass-prefixes`), **directory-listing–aware harvesting**, an **endpoint graph** (`--graph`), explicit **OpenAPI ingest** (`--openapi`), **authenticated-scan session detection** (invalid-at-start + expired-mid-scan), **method discovery** (`--probe-405`: a 405 surfaces its `Allow` header free, then opt-in POST/PATCH probing reveals the accepted write method), **scan diffing** (`--diff` — recon-over-time), **memory hygiene** (www/apex normalization, content-hash bundle filtering + ≥2-host n-gram floor, `--forget`/`--forget-noise`), public-suffix-aware scope (with **tenant confinement** on shared path-multitenant hosts like Firestore/Storage `…/projects/<id>/…`, so history/memory never pull a co-tenant's paths into scope), and full anti-WAF realism (`Retry-After` honoring, `--rotate-ua`, `--proxy-file` rotation, `--replay-proxy`, `--http2`, **weak-DH/legacy-TLS auto-fallback**), and a **multi-identity authorization differential** (`--as` — replay the discovered surface under several identities and flag BOLA/BFLA/broken-auth where a lesser identity reaches what only the privileged session should). 373 unit tests + a live integration scan.
