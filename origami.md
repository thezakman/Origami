...
- **discovery/apidocs.py** — OpenAPI/Swagger + JSON:API spec discovery and folding, **multi-document aware** (reads the Swagger-UI's `urls:[{url,name},…]` config so a .NET app serving several specs under one UI — `/swagger/internal/…`, `/swagger/siscomexEvents/…` — gets ALL of them, *and* still probes the default locations so a `/swagger/v1/swagger.json` the UI doesn't list is folded too). **Anchored at the host root + every ancestor of the target path**, so a deep-path scan (`…/api/motoristas`) still finds the root `/swagger/…`; large specs are fetched with a raised body cap so a multi-MB doc isn't truncated into unparseable JSON. Also ingests a spec handed in directly (`--openapi URL|FILE`).
- **discovery/apiver.py** — API **version pivot**: a confirmed versioned endpoint (`/v1/faq`, `/api/v2/…`) → the **whole low version band** (`v0`–`v9`, extended upward around a high current version), the legacy/next surface still wired in the backend. Sweeping the full band (not just the neighbours) catches the `/v3/`, `/v5/`, … an `/v1/` API still answers that no wordlist reaches; already-confirmed versions are deduped, so the breadth only costs a request for the ones not yet found.
...
- **discovery/wellknown.py / graphql.py / odata.py / clientapp.py / methods.py** — `.well-known/` (OIDC/OAuth + security.txt), GraphQL introspection + schema analysis (args, sensitive ops, queries-only unauth probe → `auth-bypass`), OData `$metadata`/EDMX mining (entity sets → seeds, properties → params, sensitive-surface flagging, read-only `$apply=aggregate` probe → `odata-agg`/`auth-bypass`), service-worker/web-app-manifest, HTTP-method (OPTIONS) discovery. On a **405**, the `Allow` header is surfaced free, and `--probe-405` replays with POST/PATCH (empty & `{}` body, never PUT/DELETE) to reveal the accepted write method.
- **authz.py** — passive JWT + OAuth weakness detection. A JWT in a response (`{"token":…}` body, `Set-Cookie` session, `WWW-Authenticate` header) is decoded and its header/claims flagged: `alg:none`, `kid` path-traversal / URL-injection, `jku`/`x5u` remote-key URLs (SSRF / key injection), `x5c` key-confusion, missing `exp`, privilege claims (`role`/`admin`/`scope`) — tag `jwt`. An OAuth authorize URL (`response_type`+`client_id`) is parsed for missing `state` (CSRF), PKCE `plain` downgrade, or no PKCE — tag `oauth`. Read-only: nothing is forged or replayed.
- **authz_diff.py** — **multi-identity authorization differential** (`--as`): the bridge from discovery to access-control bugs (BOLA/BFLA/broken auth — OWASP API #1/#3/#5). The discovered surface is **replayed under several identities** — the primary scan session, each `--as 'label: Header: value'` identity, and an implicit `anon` (auth stripped) — and the responses diffed. It flags CONVERGENCE where identities should DIVERGE: a lesser/unauthenticated identity reaching the SAME content as the privileged session (`broken-auth` when anon does, `bola-lead` when a distinct authed identity does), plus the inverse anomaly (`authz-diff` — the privileged session denied where a lesser one is served). Convergence is gated to non-public resources (sensitive tag / protected area / mixed verdicts) so shared public pages aren't flagged, and per-user *divergent* bodies are treated as correct scoping, not a bug. Auto-runs on an authenticated scan (free anon-vs-authed diff) or whenever `--as` is given; `--no-anon` drops the implicit anon. Read-only GETs — nothing forged or replayed onward (the Autorize/Auth-Analyzer idea, native to the discovery engine).
...
- **modules/paramfuzz.py** — reflected-parameter discovery (`--params`): harvested + common names, batched canaries, control-param FP guard, and **verified injection leads** — a breakout probe (`'"<>{{7*7}}`) confirms raw-vs-escaped reflection (`xss-lead`), template eval `{{7*7}}→49` (`ssti-lead`), and canary-in-`Location`/headers (`redirect-lead`).
...
**Integration:** drives `~/go/bin/shortscan` with `--output ndjson`, parses the result, and turns each leaked name into prioritized seeds. Flags `--shortscan` (force) / `--no-shortscan` (disable). Origami mirrors its own aggregate `--rate` into the child; under **`--deep`** (and only when the target isn't throttling — the subprocess doesn't share Origami's AIMD backoff) it drives shortscan in **`--auto`**: the full cheap→expensive long-name recovery cascade (wordlist → known-extensions → sibling → references → tilde-continuation → collision/checksum → webdav → brute stem-completion). shortscan reconstructs the FULL names itself and returns them in the `fullname` field, so they fold in as **tier-0 seeds** (higher-confidence than Origami's own n-gram Regime-2, which stays the fallback). Bounded by `--maxtime` (from the remaining scan budget); any precondition-gated source-disclosure loot (CVE-2023-36899) is redirected to a scratch dir and surfaced, never left in the CWD.
...
origami/
  ...
  modules/
    waf.py  secrets.py  leaks.py  paramfuzz.py  vhost.py  bypass403.py  cache_poison.py  session.py  authz.py  authz_diff.py
    discovery/   robots.py  js_parser.py  backups.py  shortname.py  apidocs.py
                 wellknown.py  graphql.py  odata.py  clientapp.py  methods.py  wayback.py  vcs.py  buckets.py  apiver.py  mutate.py
  brain/
...
- **shortscan** (gate + constraint-filter + raw 8.3 + prefix-as-dir/file + **n-gram Regime-2 completer**, primed by **cross-target name memory** so 8.3 prefixes reverse into names seen on past targets — the §4 learning loop; 5xx guesses and case-dupes filtered; under **`--deep`** driven in **`--auto`** for the full long-name recovery cascade → tier-0 reconstructed-name seeds, rate-mirrored + `--maxtime`-bounded, throttle-gated, loot kept out of the CWD);
- **recon phase** (single pass feeding the dynamic wordlist): **js_parser** (JS→JS chunks/sourcemaps, skips vendor, `data-main`) + **service worker** (precache) + **web app manifest** + **CSP/Link header** endpoints + **parameter** intel; **backups/VCS**; **robots/sitemap** (follows nested `<sitemapindex>` files); **OpenAPI/Swagger + JSON:API** spec discovery (**multi-document aware** — parses the Swagger-UI `urls:[…]` config and folds EVERY declared spec, incl. custom per-area paths a brute list would miss); **`.well-known/`** (OIDC/OAuth auth-endpoint folding + security.txt); **GraphQL** (introspection confirms the endpoint, then reads the schema: extracts query/mutation **arguments** as input surface, flags **sensitive operations** (login/token/senha/redefinir/lgpd/boleto/…), and sends a benign no-arg query — **queries only, never mutations** — for the top ops to flag which respond **without auth** as `auth-bypass`/BOLA leads, the GraphQL analog of probing which Swagger paths answer unauthenticated); **OData** (the `$metadata`/EDMX schema — or the JSON service document — is mined: entity sets fold in as seeds, property names enrich the param surface, Functions/Actions and sensitive sets/ops are flagged, and a **read-only** `$apply=aggregate($count)` probe on the top sets flags **authorization-by-aggregation** leaks — a count returned without auth over rows you can't read individually — as `odata-agg`/`auth-bypass`; `$batch`/Actions/writes are never touched); **OData query-option exposure** (independent of `$metadata`: on any discovered API collection, a read-only `$apply=aggregate($count)` + `$top=1` probe — a count or a single row returned **without auth** is an authorization-bypass data-exposure lead, strongest where the plain listing is BLOCKED (413 "entity too large" / 403) but OData paging/aggregation walk around a size/access guard; reports the leaked count and the record's sensitive field NAMES, never the values; `$top=1` reads one row, never a bulk dump); **HTTP method discovery** (OPTIONS → flags PUT/DELETE/TRACE/PATCH/WebDAV); **403/401 bypass** (`--bypass-403` + `--bypass-headers` + `--bypass-prefixes`: path/header/method/case tricks, **character percent-encoding** (encode a path letter single/double `%6E`/`%256E` so a WAF regex on the literal word misses while the server decodes), and a **matrix-param management bypass** `/<route>/;/actuator/*` on curated/discovered/operator route mounts, surviving 2xx-with-content reported); **content intelligence** (`leaks.py`: stack traces, framework debug pages, internal IP/host disclosure, tagged `leak`); **parameter discovery + verified injection leads** (`--params`: reflected-canary fuzzing, then a breakout probe grades it — raw-unescaped `xss-lead`, `{{7*7}}→49` `ssti-lead`, `Location`/header reflection `redirect-lead`); **multi-identity authorization differential** (`--as 'label: Header: value'`: replay the discovered surface under the primary session + each `--as` identity + an implicit `anon`, then flag where a lesser identity reaches what only the privileged session should — `broken-auth` (anon reaches authed content), `bola-lead` (a distinct authed identity does), and the inverse `authz-diff` (privileged session denied where a lesser one is served); gated to non-public resources, per-user divergent bodies treated as correct scoping; auto-runs on an authed scan, `--no-anon` to drop anon; read-only); **historical URLs** (`--wayback`/`--gau`: Wayback CDX + Common Crawl + urlscan.io + AlienVault OTX, background-fetched); **virtual-host discovery** (`--vhost`); **origin-IP discovery + IP-based WAF bypass** (`--origin`: resolve A/AAAA + candidate origin IPs via keyed OSINT (Shodan/SecurityTrails/Censys) or keyless crt.sh, then request each IP directly with the target Host — an edge-blocked path that opens on an IP is a real bypass); **web cache poisoning** (`--cache-poison`: passive cache-layer fingerprint + safe unkeyed-input probing on throwaway cache-busters, reporting the poisoning primitive without touching the real key); **method discovery** (a 405 surfaces its `Allow` header free; `--probe-405` replays POST/PATCH with empty/`{}` body to reveal the accepted write method, never PUT/DELETE); **VCS/metadata tree reconstruction** (a leaked `.git/index`/`.DS_Store`/`.svn/wc.db` enumerated into its whole file tree); **source-map reconstruction** (`sourcesContent` mined for the original routes); **API version pivot** (`/v1/faq` → the whole `v0`–`v9` band, extended around a high current version — deduped against already-found versions); **naming-convention mutation** (`/user` → `/users`, `data.json` → `data.xml`); **cloud bucket discovery** (`--buckets`: S3/GCS/Azure refs surfaced free, listability probed on demand); **config → seeds** (same-host paths inside read configs become candidates); **feeds & sitemap variants** (RSS/Atom + `sitemap_index`); **directory-listing–aware harvest** (parse the autoindex, probe only what it hides); **Apache mod_negotiation / MultiViews** (a `300 Multiple Choices` for an extensionless name lists the real files — `/composer` → `/composer.json`+`.lock` — reported as `negotiation`/`disclosure` and the true filenames folded as seeds); **path regression / climb** (a deep target `…/reports/2024/summary.pdf` scans its directory, fetches the file, and probes every ancestor dir up to root — segments fold into the vocabulary);
- **vocabulary folding** (names+extensions from references + host/subdomain/path);
- **SQLite memory**: **k-NN over fingerprint vectors** + **association mining** + cross-target priming + `--history`; **scan diffing** (`--diff`: current scan vs the last stored run of the host — new / gone / newly-accessible `403→200`, i.e. recon-over-time / attack-surface monitoring);
